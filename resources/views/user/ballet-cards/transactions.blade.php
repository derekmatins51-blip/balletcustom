...
@section('title', 'Ballet Card Transactions')
    ...
    <div class="max-w-8xl mx-auto p-4 lg:p-6 space-y-6">
        ...
        <div class="lg:hidden mb-2">
            @include('partials._mobile_header', [
                ...
                'showBackButton' => true,
                'backUrl' => route('user.ballet-cards.view', $balletCard),
        ...
        <div class="hidden lg:block mb-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 mb-2">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Ballet Card Transactions</h1>
                        <p class="text-gray-600 dark:text-gray-400">View all transactions for your Ballet Card (Serial: {{ $balletCard->serial_number ?: 'N/A' }})</p>
                    </div>
                </div>
                <a href="{{ route('user.ballet-cards.view', $balletCard) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm hover:bg-gray-50 dark:hover:bg-gray-600/70 transition-all duration-300">
            ...
            <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-gray-700/50 p-4">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $balletCard->currency }} {{ number_format($totalSpending ?? 0, 2) }}</h3>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
            ...
            <div class="p-4">
                <form action="{{ route('user.ballet-cards.transactions', $balletCard) }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-right">
                                        <span class="@if($transaction->transaction_type == 'purchase') text-red-600 dark:text-red-400 
                                            ...
                                            @elseif(in_array($transaction->transaction_type, ['refund', 'funding']))
                                                +{{ $balletCard->currency }} {{ number_format(abs($transaction->amount), 2) }}
                                            @else
                                                {{ $balletCard->currency }} {{ number_format($transaction->amount, 2) }}
                                            @endif
                                        </span>
                                    </td>
                                </tr>
